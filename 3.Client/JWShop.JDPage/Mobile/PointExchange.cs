   ...
   public class PointExchange:CommonBasePage
    {
        ...
        protected override void PageLoad()
        {
            ...
            if (base.UserId <= 0)
            {
                ResponseHelper.Redirect("/mobile/user/login.html?RedirectUrl=/mobile/pointexchange/" + id.ToString() + ".html");
                ResponseHelper.End();
            }

            gift = PointProductBLL.Read(id);
            if (gift.EndDate.Date < DateTime.Now.Date) ResponseHelper.Redirect("/mobile/pointproduct.html");

            var currentUser = UserBLL.ReadUserMore(base.UserId);
