...
namespace JWShop.Page
{
    public class BasePage : IHttpHandler, IRequiresSessionState
    {
        ...
        protected void CheckUserLogin(int t=0)
        {
            if (t == 0)
            {
                if (UserId <= 0 || UserBLL.Read(UserId).Id<=0)
                {
            ...
            else
            {
                if (UserId <= 0 || UserBLL.Read(UserId).Id <= 0)
                {
                    ResponseHelper.Write("<script language='javascript'>window.location.href='/Mobile/User/login.html?RedirectUrl=" + Request.RawUrl + "';</script>");
                    ResponseHelper.End();
                }
            }
        }
        ...
        {
            return Cookies.Admin.GetAdminID(true) != 0;
        }
    }
}
