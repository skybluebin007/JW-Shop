...
namespace JWShop.Page.Lab
{
    public class CartAjax : AjaxBasePage
    {
        ...
        /// 改变下单数量
        /// </summary>
        private void ChangeBuyNumber()
        {
            //string strCartID = StringHelper.SearchSafe(RequestHelper.GetQueryString<string>("StrCartID"));
            //int buyCount = RequestHelper.GetQueryString<int>("BuyCount");
            //string optType = StringHelper.SearchSafe(RequestHelper.GetQueryString<string>("opType"));
            //if (optType == "Remove")
            //    buyCount = 0 - buyCount;
            //string checkProduct = StringHelper.SearchSafe(RequestHelper.GetQueryString<string>("checkProduct"));
            //decimal price = RequestHelper.GetQueryString<decimal>("Price");
            //decimal productWeight = RequestHelper.GetQueryString<decimal>("ProductWeight");
            //Sessions.ProductTotalPrice += (buyCount) * price;
            //Sessions.ProductTotalWeight += (buyCount) * productWeight;
            //Sessions.CheckProduct = checkProduct;
            //ResponseHelper.Write(strCartID + "|" + Sessions.ProductBuyCount.ToString() + "|" + Sessions.ProductTotalPrice.ToString() + "|" + (buyCount * price).ToString() + "|" + buyCount.ToString());
            //ResponseHelper.End();
        }
        /// <summary>
        /// 删除购物车
