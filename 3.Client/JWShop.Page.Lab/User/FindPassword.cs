...
namespace JWShop.Page.Lab
{
    public class FindPassword : CommonBasePage
    {
        ...
        protected override void PageLoad()
        {
            ...
            if (action == "CheckSafeCode") this.CheckSafeCode();
        ...
        protected override void PostBack()
        {
            ...
            {
                ResponseHelper.Redirect("/user/findpassword.html?step=1&msg=您输入的账户名不存在，请重新输入。");
            }
            ...
            if (step <= 1)
            {
                ResponseHelper.Redirect("/user/findpassword.html?step=2&u=" + user.UserName);
            }
            //提交“验证身份”步骤
            if (step == 2)
            {
                ...
                {
                    ResponseHelper.Redirect("/user/findpassword.html?step=2&u=" + user.UserName + "&msg=您输入的短信验证码有误，请重新获取。");
                }

                ResponseHelper.Redirect("/user/findpassword.html?step=3&u=" + user.UserName);
            }
            //提交“设置新密码”步骤
            if (step == 3)
            {
                ...
                ResponseHelper.Redirect("/user/findpassword.html?step=4");
            }
