function fatherUnlimitClassChange(i, prefix,functionName) {   
    ...
    if(childData.length>0){      
        ...
        if (i == 2) { newSelectObj.options.add(new Option("请选择", 0)); } else {
...
function addUnlimitClass(prefix) {
    ...
    if (text != "" && idList != "") {
        ...
        if (fatherExist) {
            layer.msg("不能选择该分类,该类或者其父类已存在");
        }
        else {
            if (productClassValue.indexOf("|" + idList) > -1) {
                layer.msg("不能选择该分类，该类的子类已存在");
            }
            else {
                text = text.substr(0, text.length - 1);
                var content = text + "&nbsp;<a href=\"javascript:deleteUnlimitClass('|" + idList + "','" + prefix + "')\" style=\"display: inline-block;vertical-align: middle;line-height: 12px;\" title=\"删除\"><img src=\"static/images/ico-delete.png\" /></a>";
                var pElemen = document.createElement("p");
                //with (pElemen) {
                    if (prefix) {
                        pElemen.id = prefix + "SelectUnlimitClass|" + idList;
                    }
                    else {
                        pElemen.id = "SelectUnlimitClass|" + idList;
                    }
                //}
