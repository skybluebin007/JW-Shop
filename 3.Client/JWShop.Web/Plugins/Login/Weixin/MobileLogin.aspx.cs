...
namespace JWShop.Login.Weixin
{
    ...
    {     
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            //成功授权后的回调地址
            string redirect_url = Server.UrlEncode("http://" + Request.Url.Host + "/Plugins/Login/Weixin/MobileWeixinLogin.aspx");
            string _state = new Random().Next(0, 9999).ToString();
            //微信授权登录 scope:snsapi_userinfo
            //微信网页应用 scope:snsapi_login
            string weixin_url = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=" + app_id + "&redirect_uri=" + redirect_url + "&response_type=code&scope=snsapi_userinfo&state=" + _state + "#wechat_redirect";
