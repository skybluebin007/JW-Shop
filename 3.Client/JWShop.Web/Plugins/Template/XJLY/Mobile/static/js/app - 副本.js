...
application.prototype = {
	...
	checkouts: function(){
		...
		// 选择项目
		$('#paytypeRadio .radio dd, #addressRadio .radio dt').bind('click', function(){
		...
		$('body').on('click', '#listCar .del', function () {
			...
			_self.jConfirm('确认要删除此商品吗？', function(res){
			    if (res) {
                    ...
                    $.get("/shop/CartAjax.aspx?Action=DeleteCart&StrCartID=" + strCartID + "&OldCount=" + oldCount + "&Price=" + price + "", function () {
                        dealDeleteCart();
                    });				    
				}
			});
	...
	nav: function(){
		...
		$navSide.find('dd').bind('click', function(){
			$(this).addClass('current').siblings().removeClass('current');
			var index = $(this).index(),
				_height = $(this).height() + 1;
