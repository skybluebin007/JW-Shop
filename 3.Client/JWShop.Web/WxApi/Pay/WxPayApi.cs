    ...
    public class WxPayApi
    {
        ...
        */
        public static WxPayData OrderQuery(WxPayData inputObj, int timeOut = 6)
        {
            ...
            inputObj.SetValue("appid", WxPayConfig.SERVERAPPID);//公众账号ID
            inputObj.SetValue("mch_id", WxPayConfig.SERVERMCHID);//商户号
            inputObj.SetValue("sub_appid", WxPayConfig.APPID);//公众账号ID
            inputObj.SetValue("sub_mch_id", WxPayConfig.MCHID);//商户号
            //inputObj.SetValue("appid", WxPayConfig.APPID);//公众账号ID
            //inputObj.SetValue("mch_id", WxPayConfig.MCHID);//商户号
            ...
            Log.Error("WxPayApi", "OrderQuery request : " + xml);
            string response = HttpService.Post(xml, url, false, timeOut);//调用HTTP通信接口提交数据
            Log.Error("WxPayApi", "OrderQuery response : " + response);
        ...
        */
        public static WxPayData UnifiedOrder(WxPayData inputObj, int timeOut = 6)
        {
            ...
            inputObj.SetValue("sub_appid", WxPayConfig.APPID);//公众账号ID
            inputObj.SetValue("sub_mch_id", WxPayConfig.MCHID);//商户号
            //inputObj.SetValue("appid", WxPayConfig.APPID);//公众账号ID，非服务商使用
            //inputObj.SetValue("mch_id", WxPayConfig.MCHID);//商户号，非服务商使用
            //inputObj.SetValue("spbill_create_ip", WxPayConfig.IP);//终端ip	  	    
            inputObj.SetValue("nonce_str", GenerateNonceStr());//随机字符串
