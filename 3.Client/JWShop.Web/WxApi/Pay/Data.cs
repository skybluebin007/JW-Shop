...
namespace JWShop.XcxApi.Pay
{
    ...
    public class WxPayData
    {
        ...
        */
        ...
        /// <summary>
        /// 将xml转为WxPayData对象并返回对象内部的数据-重载：checksign 是否需要检查签名
        /// </summary>
        /// <param name="xml">待转换的xml串</param>
        /// <param name="checksign">是否需要检查签名</param>
        /// <returns></returns>
        public SortedDictionary<string, object> FromXml(string xml, bool checksign)
        {
            if (string.IsNullOrEmpty(xml))
            {
                Log.Error(this.GetType().ToString(), "将空的xml串转换为WxPayData不合法!");
