...
namespace JWShop.XcxApi.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            //是否授权获取基本信息
            bool hasPhoto = !string.IsNullOrEmpty(user.Photo.Trim());
            ...
            #endregion   
            //计算是否
            return Json(new { ordercount = arrT, hasPhoto = hasPhoto, hasmobile = hasMobile, usergrade = userGradeInfo, cangetbirthdaycoupon = canGetBirthdayCoupon, couponleft = user.CouPonLeft, pointleft = user.PointLeft, userInfo = user }, JsonRequestBehavior.AllowGet);
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult BindWxUserInfo()
        {
            int uid = RequestHelper.GetForm<int>("uid");
            if (user.Id <= 0 || user.Id != uid) { return Json(new { ok = false, msg = "无效的请求" }); }
            else
            {

                string username = RequestHelper.GetForm<string>("username");
                //写入数据表前编码
                username = System.Web.HttpUtility.UrlEncode(username, System.Text.Encoding.UTF8);
                string photo = RequestHelper.GetForm<string>("photo");
                int sex = RequestHelper.GetForm<int>("sex") <= 0 ? 3 : RequestHelper.GetForm<int>("sex");
                Dictionary<string, object> dict = new Dictionary<string, object>();
                dict.Add("[UserName]", username);
                dict.Add("[Photo]", photo);
                dict.Add("[Sex]", sex);
                UserBLL.UpdatePart("[usr]", dict, user.Id);
                return Json(new { ok = true });
