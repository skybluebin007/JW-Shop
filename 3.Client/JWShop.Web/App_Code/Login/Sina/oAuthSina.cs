    ...
    public class oAuthSina : oAuthBase
    {
        ...
        public string appSecret
        {
            get
            {
                ...
                {
                    _appSecret = loginConfig.AppSecret;
                }
                return _appSecret;
            }
        ...
        public int AccessTokenGet()
        {
            ...
            if (response.Length > 0)
            {
                ...
                if (qs["user_id"] != null)
                {
                    userID = Convert.ToInt32(qs["user_id"]);
                }
            }
            return userID;
        }
        ...
        {
            string url = "http://api.t.sina.com.cn/users/show/" + userID.ToString() + ".xml";
            string result = oAuthWebRequest(Method.GET, url, string.Empty);
            XmlDocument xd = new XmlDocument();
            xd.LoadXml(result);
            return xd.SelectSingleNode("user/name").InnerText;
        }
        /*Upload a message*/
        public string oAuthWebRequestWithPic(Method method, string url, string postData, string filepath)
        {
            ...
            string oauthSignaturePattern = "OAuth oauth_consumer_key=\"{0}\", oauth_signature_method=\"HMAC-SHA1\",oauth_timestamp=\"{1}\", oauth_nonce=\"{2}\", oauth_version=\"1.0\", oauth_token=\"{3}\",oauth_signature=\"{4}\"";

            string contentEncoding = "iso-8859-1";

            string normalizedString, normalizedParameters;
            string timestamp = this.GenerateTimeStamp();
            string nounce = this.GenerateNonce();
