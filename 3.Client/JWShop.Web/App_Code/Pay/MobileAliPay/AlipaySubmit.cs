...
namespace JWShop.Pay.MobileAliPay
{
    /// <summary>
    /// 类名：Submit
    /// 功能：支付宝各接口请求提交类
    ...
    public class Submit
    {
        ...
        static Submit()
        {
            _key = PayConfig.Key.Trim();
            _input_charset = PayConfig.Input_charset.Trim().ToLower();
            _sign_type = PayConfig.Sign_type.Trim().ToUpper();
        }
        ...
        {
            string url = GATEWAY_NEW + "service=query_timestamp&partner=" + PayConfig.Partner + "&_input_charset=" + PayConfig.Input_charset;
            string encrypt_key = "";
